...
-- v3.1.1 (2026-03-30): Birthday Group — Calendar source (ground truth)
--   handleBirthdayGroup() rewritten: Calendar Birthday calendar replaces CNContactStore scan.
--   Bridge: addressbook://UUID in event URL → direct contact lookup (no name matching).
--   New Phase 10: improperly-filled birthday report (Calendar hit + no birth date in Contacts).
--   Fixes: ASOC scan missed ~40% contacts from non-primary accounts.
-- v3.1.0 (2026-03-30): Birthday Group Management (CNContactStore-based, superseded by v3.1.1)
--   More menu: split "🎂 Refresh Birthday Cache" → "🎂 Manage Birthday Group" + "🔄 Refresh Birthday Cache"
--   Helpers: lsamBirthdayLabel()
...
property _pVersion : "3.1.1" -- single source of truth for window title
...
-- ── v3.1.0 helper: return window label for an MM-DD string ───────────────────
-- windowDates is a list: {yesterday, today, tomorrow, T+2} as "MM-DD" strings
on lsamBirthdayLabel(mmdd, windowDates)
	set labels to {"[YESTERDAY]", "[TODAY]", "[TOMORROW]", "[T+2]"}
	repeat with i from 1 to 4
		if mmdd is equal to item i of windowDates then
			return item i of labels
		end if
	end repeat
	return "[" & mmdd & "]"
end lsamBirthdayLabel

-- ── v3.1.1 handler: manage LSAM-Birthday group (Calendar source) ─────────────
-- Source: Calendar "Birthdays" (locale-safe) — OS-level ground truth for macOS birthday events.
-- Bridge: addressbook://UUID in event URL → direct Contacts.app lookup (zero ambiguity).
-- Window: [T-1, T, T+1, T+2] — covers manual backfill + birthday_trigger.py automation window.
-- Phase 10: reports contacts visible in Calendar but missing birth date in Contacts.app.
on handleBirthdayGroup()
	-- ── Phase 1: Build 4-day window [T-1, T, T+1, T+2] ─────────────────────────
	set yestMMDD to do shell script "date -v-1d '+%m-%d'"
	set todayMMDD to do shell script "date '+%m-%d'"
	set tomMMDD to do shell script "date -v+1d '+%m-%d'"
	set t2MMDD to do shell script "date -v+2d '+%m-%d'"
	set windowDates to {yestMMDD, todayMMDD, tomMMDD, t2MMDD}

	set todayStart to current date
	set time of todayStart to 0
	set windowStart to todayStart - 86400
	set windowEnd to todayStart + 259199

	-- ── Phase 2: Locate Birthday calendar (locale-safe) ─────────────────────────
	-- Tries common locale names; falls back to substring search on "irth"/"nniversaire".
	set bdCal to missing value
	tell application "Calendar"
		repeat with tryName in {"Birthdays", "Anniversaires", "Geburtstage", "Compleanni", "Cumpleanos"}
			try
				set bdCal to calendar (tryName as text)
				exit repeat
			end try
		end repeat
		if bdCal is missing value then
			repeat with c in every calendar
				set cn to name of c
				if cn contains "irth" or cn contains "nniversaire" then
					set bdCal to c
					exit repeat
				end if
			end repeat
	...
	if bdCal is missing value then
		display dialog "Birthday calendar not found in Calendar.app." & return & "Enable it under Calendar > Settings > Accounts." with title "LSAM — Birthday Group" buttons {"OK"} default button "OK"
		return
	end if

	-- ── Phase 3: Fetch Birthday calendar events, collect + filter per-event ────────
	-- `start date` is a two-word property; batch access via `start date of list`
	-- triggers a parse conflict with the `start` keyword — must access per-event.
	-- Collect raw data inside the tell block, then filter by date window outside.
	set rawEvtNames to {}
	set rawEvtURLs to {}
	set rawEvtDates to {}
	-- `tell evt` nested form resolves `start date`/`summary`/`url` unambiguously
	-- when both Calendar + Contacts dictionaries are loaded (avoids multi-dict ambiguity).
	tell application "Calendar"
		set allBdEvts to every event of bdCal
		repeat with evt in allBdEvts
			tell evt
				set end of rawEvtDates to start date
				set end of rawEvtNames to (summary as text)
				set rawURL to ""
				try
					set rawURL to (url as text)
				end try
				set end of rawEvtURLs to rawURL
			end tell
		end repeat
	end tell

	-- Filter to window [T-1 .. T+2] outside Calendar tell block
	-- `time of dateVar` is safe here (not shadowed by use framework imports)
	set evtNames to {}
	set evtURLs to {}
	set evtStartDates to {}
	repeat with i from 1 to count of rawEvtDates
		set evtDate to item i of rawEvtDates
		set evtMidnight to evtDate
		set time of evtMidnight to 0
		if evtMidnight >= windowStart and evtMidnight <= (todayStart + 172800) then
			set end of evtNames to item i of rawEvtNames
			set end of evtURLs to item i of rawEvtURLs
			set end of evtStartDates to evtDate
		end if
	end repeat

	-- ── Phase 4: Resolve contact IDs from addressbook:// URLs ────────────────────
	-- Date→MM-DD via arithmetic (avoids `month`/`day` keywords shadowed by use framework)
	set calContactIDs to {}
	set calContactDisplayNames to {}
	set unresolvedEvents to {}

	repeat with i from 1 to count of evtNames
		set evtName to item i of evtNames
		set evtURL to item i of evtURLs
		set evtDate to item i of evtStartDates

		-- Map event date → window slot by date arithmetic (no month/day keywords)
		set evtMidnight to evtDate
		set time of evtMidnight to 0
		set evtMMDD to "??"
		if evtMidnight = windowStart then
			set evtMMDD to yestMMDD
		else if evtMidnight = todayStart then
			set evtMMDD to todayMMDD
		else if evtMidnight = (todayStart + 86400) then
			set evtMMDD to tomMMDD
		else if evtMidnight = (todayStart + 172800) then
			set evtMMDD to t2MMDD
		end if

		-- "addressbook://" is 14 chars; text 15 thru -1 extracts the UUID
		if evtURL starts with "addressbook://" then
			set contactUUID to text 15 thru -1 of evtURL
			if contactUUID is not "" then
				set end of calContactIDs to contactUUID
				set end of calContactDisplayNames to evtName & "  " & my lsamBirthdayLabel(evtMMDD, windowDates)
			else
				set end of unresolvedEvents to evtName & " [empty UUID]"
			end if
		else
			set end of unresolvedEvents to evtName & " [no addressbook URL]"
		end if
	end repeat

	-- ── Phase 5: Get current LSAM-Birthday group members ────────────────────────
	set currentMemberIDs to {}
	set toRemoveIDs to {}
	set toRemoveNames to {}

	tell application "Contacts"
		set bdGroup to missing value
		repeat with g in every group
			if name of g is "LSAM-Birthday" then
				set bdGroup to g
				exit repeat
			end if
		end repeat
		if bdGroup is not missing value then
			set groupMembers to every member of bdGroup
			repeat with p in groupMembers
				set pID to id of p
				set pName to name of p
				set end of currentMemberIDs to pID
				-- Stale: in group but not in Calendar window
				if calContactIDs does not contain pID then
					set end of toRemoveIDs to pID
					set end of toRemoveNames to pName & "  [STALE]"
				end if
			end repeat
		end if
	end tell

	-- ── Phase 6: Compute diffs (to-add vs already-present) ──────────────────────
	set toAddIDs to {}
	set toAddDisplayNames to {}
	set alreadyPresentNames to {}
	repeat with i from 1 to count of calContactIDs
		set cID to item i of calContactIDs
		if currentMemberIDs contains cID then
			set end of alreadyPresentNames to item i of calContactDisplayNames
		else
			set end of toAddIDs to cID
			set end of toAddDisplayNames to item i of calContactDisplayNames
		end if
	end repeat

	-- ── Phase 7: Detect improperly-filled birthdays ───────────────────────────────
	-- Contact appears in Calendar Birthday but has no birth date set in Contacts.app.
	-- Typical cause: birthday came from Exchange/CardDAV and was never entered manually.
	set noDateContactNames to {}
	tell application "Contacts"
		repeat with i from 1 to count of calContactIDs
			try
				set p to first person whose id is item i of calContactIDs
				if birth date of p is missing value then
					set end of noDateContactNames to item i of calContactDisplayNames
				end if
			end try
		end repeat
	end tell

	-- ── Phase 8: Build dry-run dialog ─────────────────────────────────────────────
	set windowStr to yestMMDD & " / " & todayMMDD & " / " & tomMMDD & " / " & t2MMDD
	set addCount to count of toAddIDs
	set removeCount to count of toRemoveIDs
	set presentCount to count of alreadyPresentNames

	set dialogMsg to "Birthday Group Management" & return
	set dialogMsg to dialogMsg & "Window: " & windowStr & return & return

	set dialogMsg to dialogMsg & "Already in group (" & presentCount & "):" & return
	if presentCount > 0 then
		repeat with n in alreadyPresentNames
			set dialogMsg to dialogMsg & "  * " & n & return
		end repeat
	else
		set dialogMsg to dialogMsg & "  (none)" & return
	end if

	set dialogMsg to dialogMsg & return & "To add (" & addCount & "):" & return
	if addCount > 0 then
		repeat with n in toAddDisplayNames
			set dialogMsg to dialogMsg & "  + " & n & return
		end repeat
	else
		set dialogMsg to dialogMsg & "  (none)" & return
	...
	set dialogMsg to dialogMsg & return & "Stale to remove (" & removeCount & "):" & return
	if removeCount > 0 then
		repeat with n in toRemoveNames
			set dialogMsg to dialogMsg & "  - " & n & return
		end repeat
	else
		set dialogMsg to dialogMsg & "  (none)" & return
	...
	-- ── Phase 9: Execute or dismiss ───────────────────────────────────────────────
	if addCount is 0 and removeCount is 0 then
		display dialog dialogMsg with title "LSAM — Birthday Group (Dry Run)" buttons {"OK"} default button "OK"
	else
		set dialogResult to button returned of (display dialog dialogMsg with title "LSAM — Birthday Group (Dry Run)" buttons {"Cancel", "Populate Group"} default button "Populate Group")
		if dialogResult is "Populate Group" then
			set addedCount to 0
			...
			tell application "Contacts"
				...
				if bdGroup is missing value then
					display dialog "LSAM-Birthday group not found. Please create it first." with title "LSAM — Birthday Group" buttons {"OK"} default button "OK"
					return
				end if
				repeat with i from 1 to count of toAddIDs
					try
						set p to first person whose id is item i of toAddIDs
						add p to bdGroup
						set addedCount to addedCount + 1
					end try
				end repeat
				repeat with i from 1 to count of toRemoveIDs
					try
						set p to first person whose id is item i of toRemoveIDs
						remove p from bdGroup
						set removedCount to removedCount + 1
					end try
				end repeat
				save
			end tell
			display notification "Birthday group: +" & addedCount & " added, -" & removedCount & " removed." with title "LSAM"
		end if
	end if

	-- ── Phase 10: Improperly-filled birthday report (informational, always shown) ─
	set noDateCount to count of noDateContactNames
	if noDateCount > 0 then
		set reportMsg to noDateCount & " contact(s) appear in Calendar Birthday but have no birth date in Contacts.app:" & return & return
		repeat with n in noDateContactNames
			set reportMsg to reportMsg & "  * " & n & return
		end repeat
		set reportMsg to reportMsg & return & "Consider adding their birth date in Contacts.app for complete tracking."
		display dialog reportMsg with title "LSAM — Missing Birth Dates (" & noDateCount & ")" buttons {"OK"} default button "OK"
	end if

end handleBirthdayGroup
...
on mainDashboard()
	...
	repeat
		...
		if choice starts with "📋 1." then
			...
			set moreItems to {"🚀 Promote Selection → Queue", "🛑 Demote Selection ← Queue", "✏️ Edit Override (by name)", "🔍 Inspect Contact Archive", "🧬 Profile Review (by name)", "🎂 Manage Birthday Group", "🔄 Refresh Birthday Cache", "📊 Status", "🚪 Back"}
			...
			if morePick is not false then
				...
				if moreChoice starts with "🚀" then
					...
					-- v3.1.1: Manage Birthday Group (Calendar source, moving window add/remove)
					my handleBirthdayGroup()
				else if moreChoice starts with "🔄" then
